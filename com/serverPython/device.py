...
class DeviceSerial():
	""" device connecté via un port série
	"""
	...
	def __str__(self):
		return "DeviceSerial(%s)"%self.origin
	
	def close(self):
		self._serial.close()


class DeviceSubprocess():
	""" device connecté via un pipe
	"""
	...
	def __str__(self):
		return "DeviceSubprocess(%s)"%self.origin
